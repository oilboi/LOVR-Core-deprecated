...
function generate_chunk_vertices(chunk_x,chunk_z)
    ...
    if #chunk_vertices > 0 then
        -- This mesh is a cube
        chunk_stack[#chunk_stack+1] = lovr.graphics.newMesh({{ 'lovrPosition', 'int', 3 },{ 'lovrTexCoord', 'float', 2 }}, chunk_vertices, 'triangles', "static")

        chunk_stack[#chunk_stack]:setVertexMap(chunk_indexes)
    end
